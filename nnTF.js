...
console.log(typeof(pesosCapaUno))
console.log(typeof(biasCapaUno))
...
async function entrena(iteraciones) {
    ...
    for (let i = 0; i < iteraciones; i++) {
        ...
        if (i % 100 === 0) {
          ...
          document.getElementById('divEntrenamiento').innerHTML += 'Loss['+i+']: '+ costods +'<br>'
...
function updateCiclosDeAprendizaje(ciclos){
    document.getElementById('divCiclosDeAprendizaje').innerHTML="Learning cycles: " + ciclos;
}
...
async function aprendeXor() {
    ...
    document.getElementById('divEntrenamiento').innerHTML += '<br>Number of training iterations (random): '+iteraciones+'<br><br>'
    const loss = await entrena(iteraciones);
    const time = performance.now() - timeStart;
    document.getElementById('divEntrenamiento').innerHTML += '<br>Loss: ' + loss[0]+'<br><br>'
    document.getElementById('divEntrenamiento').innerHTML += 'Duration : '+ Math.round(time/1000).toFixed(2) + ' seconds</br><br>'
}
...
async function pruebaXor() {
    ...
    document.getElementById('divPrueba').innerHTML = '<br>Test duration : '+ time2.toFixed(3) + ' milliseconds</br><br>'
